...
static void publisher(AWS_IoT_Client *client, char *base_topic, uint16_t base_topic_len){
    ...
    //cJSON *Temp_sensor = cJSON_CreateString("BODY_TEMPERATURE");
    cJSON *Temp_level = cJSON_CreateNumber(temperature);
    //cJSON_AddItemToObject(payload, "Temp_sensor", Temp_sensor);
    cJSON_AddItemToObject(payload, "T-value= 36",Temp_level);
